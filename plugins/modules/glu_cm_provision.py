...
DOCUMENTATION = '''
    module: glu_cm_provision
    short_description: Perform a Config Modeling Provision on the device.
    description:
    - For the current Gluware device execute a Config Modeling provision of currently attached model.
...
- name: Execute a Provision of a Config Model
  glu_cm_provision:
    ...
    description: "Provision of Config Model"
'''

RETURN = r'''
msg:
  description: Gluware output summary of config modeling provision
  returned: always
  type: dict
'''
...
def run_module():
    ...
    # Create API client (used for work polling/output and optional org check)
    ...
    api_url = urljoin(api_host, '/api/actions/provision')
    api_data = {
    ...
    # Execute provision via unified http_request
    ...
    if work_state == "SUCCESSFUL":
        job = glu_api._get_work_output(work_id, "provision")
        merged = {}
        ...
        module.fail_json(msg="Provision work did not complete successfully (state: {})".format(work_state), changed=False)
