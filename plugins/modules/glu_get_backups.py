...
DOCUMENTATION = '''
    module: glu_get_backups
    short_description: Gather all available backups for a Gluware Device
    description:
    - For the current Gluware device view all available backups in Gluware Control.
...
EXAMPLES = r'''
- name: View Available Backups
  gluware_inc.control.glu_get_backups:
...
RETURN = r'''
msg:
  description: Gluware backup list
  returned: always
  type: dict
'''
...
def run_module():
    ...
    # Build API URL
    api_url = urljoin(api_host, '/api/devices/{}/backup'.format(glu_device_id))

    # Call API
    res = http_request(module, api_url, "GET", headers=base_headers)
    if res["status"] not in (200, 204):
        body_txt = res.get("body", "") or res.get("reason", "")
        module.fail_json(
            msg="Unexpected response from Gluware Control (get backups): HTTP {} - {}".format(
                res["status"], body_txt
            ),
            changed=False
        )

    # Parse response body as JSON if possible; otherwise return raw text
    backups = None
    try:
        backups = json.loads(res["body"] or "[]")
    except Exception:
        backups = res["body"] or []

    result = dict(changed=False, msg=backups)
